...
const warehouseSchema = z.object({
    ...
    default_rules_profile: z.string().default("STANDARD"),
    total_area_sqft: z.coerce.number().min(0, "Cannot be negative").default(0),
    total_volume_cc: z.coerce.number().min(0, "Cannot be negative").default(0),
    max_pallet_positions: z.coerce.number().min(0, "Cannot be negative").default(0),
...
const WarehouseManagement = () => {
    ...
    const [liveBinsCount, setLiveBinsCount] = useState<number | null>(null);
    ...
    const fetchLiveCounts = async (warehouseId: string) => {
        try {
            // Fetch exact counts directly from APIs per user constraints
            const [zonesRes, binsRes] = await Promise.all([
                zoneService.getZonesByWarehouse(warehouseId),
                binsService.getBinsByWarehouse(warehouseId)
            ]);

            setLiveZonesCount(Array.isArray(zonesRes) ? zonesRes.length : (zonesRes?.items?.length || 0));
            setLiveBinsCount(Array.isArray(binsRes) ? binsRes.length : (binsRes?.items?.length || 0));
        } catch (error) {
            console.error("Failed to fetch live zone/bin counts", error);
        }
    };

    useEffect(() => {
        if (viewOpen && viewingWarehouse?.id) {
            // Reset counts for fresh modal
            setLiveZonesCount(null);
            setLiveBinsCount(null);
            fetchLiveCounts(viewingWarehouse.id);
        }
    }, [viewOpen, viewingWarehouse?.id]);
    ...
    const onEditSubmit = async (values: WarehouseFormValues) => {
        ...
        setLoading(true);
        try {
            await warehouseService.update(editingWarehouse.id || editingWarehouse.code, values);
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="e.g. 00000" {...field} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                            ...
                            <TableBody>
                                {loading && warehouses.length === 0 ? (
                                    ...
                                    filteredWarehouses.map((warehouse) => (
                                        <TableRow key={warehouse.id || warehouse.code} className="group hover:bg-primary/5 transition-colors cursor-default">
                                            ...
                                            <TableCell className="text-right pr-6">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-primary hover:bg-primary/10" onClick={() => handleView(warehouse)}>
                                                        <Eye className="w-4 h-4" />
                                                    </Button>
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-500 hover:bg-blue-500/10" onClick={() => handleEdit(warehouse)}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            ...
                            <div className="flex flex-col h-full space-y-4">
                                ...
                                <div className="flex-1 space-y-6 rounded-2xl border p-6 bg-card shadow-inner border-primary/20 flex flex-col justify-center bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-primary/5 via-transparent to-transparent">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div
                                            className="p-3 bg-background/50 rounded-xl border border-primary/10 text-center cursor-pointer hover:bg-primary/5 transition-colors group relative"
                                        ...
                                        >
                                            ...
                                            <p className="text-xl font-black text-primary">
                                                {liveZonesCount !== null ? liveZonesCount : (viewingWarehouse?.zones_count || 0)}
                                            </p>
                                        ...
                                        <div className="p-3 bg-background/50 rounded-xl border border-primary/10 text-center">
                                            <p className="text-[8px] text-muted-foreground uppercase font-bold tracking-wider mb-1">Total Bins</p>
                                            <p className="text-xl font-black text-primary">
                                                {liveBinsCount !== null ? liveBinsCount : (viewingWarehouse?.active_bins_count || 0)}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="space-y-2">
            ...
            <AddZoneModal
                ...
                onSuccess={() => {
                    ...
                    if (viewingWarehouse) {
                        setViewingWarehouse({
                            ...viewingWarehouse,
                            zones_count: (viewingWarehouse.zones_count || 0) + 1
                        });
                        fetchLiveCounts(viewingWarehouse.id);
                    }
                }}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>Default Rules Profile</FormLabel>
                                                <FormControl>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>Total Volume (cc)</FormLabel>
                                                <FormControl>
                                                    <Input type="number" min="0" {...field} />
                                                </FormControl>
                                                <FormMessage />
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>Max Pallet Positions</FormLabel>
                                                <FormControl>
                                                    <Input type="number" min="0" {...field} />
                                                </FormControl>
                                                <FormMessage />
